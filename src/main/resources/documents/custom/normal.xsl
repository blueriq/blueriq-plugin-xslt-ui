...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:svg="http://www.w3.org/2000/svg">
	...
	Date:		2010-06-07
	Description:	Normal fo-sheet for Aquima Documents

	Version history:
	20100607 1.0 M. Joosen		Normal definitions
	-->
	...
	<xsl:template match="NORMAL" mode="pagemaster">
	...
	<xsl:template match="NORMAL[@style='LABEL']" mode="pagemaster">
		<xsl:call-template name="PAGEMASTER">
			<xsl:with-param name="type" select="'NORMAL'"/>
			...
			<xsl:with-param name="pmr"  select="'25mm'"/>
			<xsl:with-param name="pmb"  select="'10mm'"/>
			<xsl:with-param name="pmt"  select="'20mm'"/>
			<xsl:with-param name="mfb"  select="'15mm'"/>
			<xsl:with-param name="mft"  select="'30mm'"/>
			...
			<xsl:with-param name="cc"   select="''"/>
			<xsl:with-param name="cg"   select="''"/>
			<xsl:with-param name="hf"   select="'25mm'"/>
